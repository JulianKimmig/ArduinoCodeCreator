...
class ArduinoBaseClass(ArduinoClass):
    ...
    memcpy = Function(name="memcpy", arguments=[uint8_t_pointer,uint8_t_pointer,uint8_t])
...
class SerialClass(ArduinoClass):
    ...
    write_buf = Function("write",[Array("buffer",uint8_t),Variable("length",uint16_t)])
...
class EepromClass(ArduinoClass):
    ...
    write =  Function("write",[Variable("adress",uint16_t),Variable("value",uint8_t)])
    ...
    update =  Function("update",[Variable("adress",uint16_t),Variable("value",uint8_t)])
